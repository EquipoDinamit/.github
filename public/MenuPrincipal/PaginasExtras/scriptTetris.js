...
class Board {
  ...
  gameUpdate() {
    if (this.isGameOver()) {
      ...
      $("#new-game").text("Presiona Enter para comenzar de nuevo");
      lostGame=true;
    }
  }
  ...
  arePositonsWithinBoard(positions) {
    for (let position of positions) {
      if (position.x >= 18 || position.y < 0 || position.y >= 10) {
        return false;
      }
    }
    return true;
...
// Modificar para botones //

let board = new Board();
const socket = io('http://ivanlb.duckdns.org/');

/* El código anterior está escuchando un mensaje del servidor. Cuando el servidor envía un mensaje, el
código ejecutará la función apropiada. */
...
/* Escuchar pulsaciones de teclas y llamar a la función apropiada. */
